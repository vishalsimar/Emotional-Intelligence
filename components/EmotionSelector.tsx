...
const EmotionSelector: React.FC<EmotionSelectorProps> = ({ emotions, onSelectEmotion, onReorder, onEditClick, onDeleteClick }) => {
  ...
  const handleDragStart = (e: React.DragEvent<HTMLButtonElement>, index: number) => {
    ...
    setTimeout(() => {
        // FIX: Add 'dragging-active' class for proper cleanup in handleDragEnd
        e.currentTarget.classList.add('opacity-50', 'scale-105', 'dragging-active');
    }, 0)
  };
  ...
  const handleDragEnd = (e: React.DragEvent<HTMLButtonElement>) => {
    ...
    setDragOverIndex(null);
    // FIX: Ensure cleanup targets the correct class
    document.querySelectorAll('.dragging-active').forEach(el => el.classList.remove('opacity-50', 'scale-105', 'dragging-active'));
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 w-full pb-20">
        {emotions.map((emotion, index) => (
          <button
            ...
            ${dragOverIndex === index ? 'border-dashed border-2 border-slate-400 dark:border-slate-500' : ''}
            `}
